...
Stage 0 of the nightly pipeline for gap_directional_trap__candidate_1_v2.

Purpose:
  Incrementally extend the daily data caches so the nightly scan sees
  the most recent closed market day.

  Data sources are shared between v1 and v2 (same OHLCV and market context).
  This file is the v2-named Stage 0 entry point for the v2 orchestrator.

  Three sub-tasks (in order):
    A0. Rebuild operational universe (pure parquet reads, no API)
    A.  Extend stock daily OHLCV parquets for the universe
    B.  Extend SPY and QQQ market daily parquets
    C.  Rebuild the market-context model CSV (pure computation, no API)
    D.  Validate that the intended signal_date is present

Usage:
  python engineering_daily_data_refresh__gap_directional_trap__candidate_1_v2.py
  python engineering_daily_data_refresh__gap_directional_trap__candidate_1_v2.py --signal-date 2026-03-27
  python engineering_daily_data_refresh__gap_directional_trap__candidate_1_v2.py --signal-date 2026-03-27 --preview
  python engineering_daily_data_refresh__gap_directional_trap__candidate_1_v2.py --full-universe
...
# Operational universe: reuse the v1 operational universe file.
# Signal filter is identical between v1 and v2; eligible tickers are the same.
OP_UNIVERSE_CSV = (
    ENG_ROOT
    / "engineering_configs"
...
# ---------------------------------------------------------------------------
# Sub-task A0: rebuild operational universe
# ---------------------------------------------------------------------------
...
def rebuild_operational_universe(full_universe: bool) -> None:
    ...
    if full_universe:
        print("  --full-universe set. Skipping A0.", flush=True)
    ...
    if not OP_UNIVERSE_BUILDER_SCRIPT.exists():
        print(
            ...
            "  [warn] Falling back to full shared universe.",
            flush=True,
        )
        return
...
# ---------------------------------------------------------------------------
# Sub-task B: market reference cache
# ---------------------------------------------------------------------------
...
def rebuild_market_context_model() -> None:
    ...
    result = subprocess.run([sys.executable, str(CONTEXT_MODEL_SCRIPT)])
    if result.returncode != 0:
        print(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Stage 0: daily data refresh for gap_directional_trap__candidate_1_v2. "
            "Extends stock + market parquets and rebuilds the market context model."
        )
    )
    parser.add_argument("--signal-date", metavar="YYYY-MM-DD", default=None)
    parser.add_argument("--preview", action="store_true",
                        help="Print extra diagnostic info. Refresh still runs.")
    parser.add_argument("--full-universe", action="store_true",
                        help="Use full shared universe instead of operational universe.")
    ...
    print("  gap_directional_trap__candidate_1_v2", flush=True)
