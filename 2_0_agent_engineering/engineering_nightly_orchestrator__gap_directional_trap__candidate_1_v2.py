...
Nightly orchestrator for gap_directional_trap__candidate_1_v2.
...
Runs the full plan_next_day_day_trade pipeline in sequence:
  ...
  Stage 1 — nightly signal scan (v2)
  Stage 2 — top_3 selection layer (v2)
  Stage 3 — Telegram delivery (v2)

Calls each module as a subprocess.
Does NOT duplicate any refresh, scan, selection, or Telegram logic.

This is the ONLY active production orchestrator.
candidate_1_v1 is reference only — do not run both in parallel.

Usage examples:
  # Preview full chain with refresh — no Telegram send
  python engineering_nightly_orchestrator__gap_directional_trap__candidate_1_v2.py --signal-date 2026-03-27 --preview
  ...
  python engineering_nightly_orchestrator__gap_directional_trap__candidate_1_v2.py --preview

  # Scan + selection only, no Telegram
  python engineering_nightly_orchestrator__gap_directional_trap__candidate_1_v2.py --signal-date 2026-03-27 --skip-telegram

  # Skip refresh (data already fresh), run scan + selection + Telegram
  python engineering_nightly_orchestrator__gap_directional_trap__candidate_1_v2.py --signal-date 2026-03-27 --skip-refresh

  # Force full shared universe for refresh + scan (debug / validation)
  python engineering_nightly_orchestrator__gap_directional_trap__candidate_1_v2.py --signal-date 2026-03-27 --full-universe --skip-telegram
...
TELEGRAM_MODULE = (
    ...
    / "notifications"
    / "telegram_delivery__gap_directional_trap__candidate_1_v2.py"
)
...
SIGNAL_PACK_GLOB = "signal_pack__gap_directional_trap__candidate_1_v2__*.csv"
...
def run_stage(stage_name: str, cmd: list) -> None:
    """Run one pipeline stage as a subprocess. Halt on failure."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Nightly orchestrator: gap_directional_trap__candidate_1_v2  "
            "(refresh -> scan -> selection -> Telegram)"
        )
    )
    parser.add_argument(
        "--signal-date",
        metavar="YYYY-MM-DD",
        default=None,
        help="Signal date to process. Defaults to latest from market context (auto-detect).",
    )
    parser.add_argument(
        "--preview",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Pass --full-universe to Stage 0 and Stage 1. "
            "Forces full shared universe (~4,700 tickers) instead of operational universe."
        ),
    )
    args = parser.parse_args()

    print("\n" + "#" * 66, flush=True)
    print("  gap_directional_trap__candidate_1_v2  |  nightly orchestrator", flush=True)
    if args.signal_date:
        print(f"  signal_date: {args.signal_date}", flush=True)
    else:
        print("  signal_date: auto-detect (Stage 0 → Stage 1 resolve)", flush=True)
    if args.preview:
    ...
    print(
        ...
        f"          Writes: signal_pack__gap_directional_trap__candidate_1_v2__YYYY_MM_DD.csv",
        flush=True,
    )
    ...
    # ------------------------------------------------------------------
    print(
        f"\n[Stage 2] Reads:  signal_pack__gap_directional_trap__candidate_1_v2__{date_tag}.csv\n"
    ...
    print(
        f"\n[Stage 3] Reads:  selected_top_3__gap_directional_trap__candidate_1_v2__{date_tag}.csv\n"
        f"          Mode:   {mode_note}",
        flush=True,
    )
